    ...
    <div class="palettes-list">
      ...
      >
        ...
        <div class="palette-actions">
          <button @click="toggleFavorite(palette.id)" class="action-button" title="Toggle favorite">
            {{ palette.favorite ? '⭐' : '☆' }}
          </button>
          <button @click="loadPalette(palette)" class="action-button" title="Load palette">
            📂
          </button>
          <button @click="deletePalette(palette.id)" class="action-button delete" title="Delete palette">
            🗑️
          </button>
        </div>
      </div>
...
const filteredPalettes = computed(() => {
  ...
  return savedPalettes.value.filter(palette => {
    ...
    const tagsMatch = palette.tags && palette.tags.some(tag => tag.toLowerCase().includes(query));
...
const savePalette = () => {
  if (!paletteName.value.trim()) {
    emit('notify', 'Please enter a palette name');
    return;
  }
  ...
  emit('notify', 'Palette saved successfully!');
};

const loadPalette = (palette) => {
  emit('load', palette.colors);
  emit('notify', `Loaded palette: ${palette.name}`);
};
