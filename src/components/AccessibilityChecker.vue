<template>
  <div class="accessibility-checker">
    <h3>Accessibility Checker (WCAG)</h3>
    ...
    <div class="contrast-result" v-if="foregroundColor && backgroundColor">
      ...
      <div class="metrics">
        ...
        <div class="metric">
          ...
          <span :class="`level level-${normalTextLevel.toLowerCase()}`">
            {{ normalTextLevel }}
          </span>
        </div>
        ...
        <div class="metric">
          ...
          <span :class="`level level-${largeTextLevel.toLowerCase()}`">
            {{ largeTextLevel }}
          </span>
        </div>
      </div>
      ...
      <div class="recommendations" v-if="normalTextLevel === 'Fail'">
        <p>⚠️ These colors don't meet WCAG standards for normal text.</p>
        <p>Consider using colors with higher contrast or adjusting lightness.</p>
      </div>
    </div>
  </div>
</template>
...
const contrastRatio = computed(() => {
  ...
  return getContrastRatio(foregroundColor.value.hex, backgroundColor.value.hex);
});
